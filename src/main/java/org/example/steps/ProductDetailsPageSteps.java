...
public class ProductDetailsPageSteps {
    ...
    @CustomStep("Validate Products Details page appearance")
    ...
    @CustomStep("Click on Back to products page")
    public void clickOnBackArrow() {
        ui.getBackArrow().click();
    }

    @CustomStep("Validate on Product Details page the title is [{title}]")
    public void validateTitle(String title) {
        assertThat(ui.getTitle()).containsText(title, new LocatorAssertions.ContainsTextOptions().setTimeout(5000));
    }

    @CustomStep("Validate on Product Details page the description is [{description}]")
    public void validateDescription(String description) {
        assertThat(ui.getDescription()).hasText(description, new HasTextOptions().setIgnoreCase(false));
    }

    @CustomStep("Validate on Product Details page the back arrow has label [{backArrow}]")
    public void validateBackArrow(String backArrow) {
        assertThat(ui.getBackArrow()).containsText(backArrow);
    }

    @CustomStep("Validate on Product Details page the price is [{price}]")
    public void validatePrice(String price) {
        assertThat(ui.getPrice()).hasText(price);
    }

    @CustomStep("Validate on Product Details page the add to cart button has text [{addToCartButton}]")
